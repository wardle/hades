...
* FHIR REST search now pages at the provider: FTRM pushes `ORDER BY url,version LIMIT` into its `WITHOUT ROWID` clustered tables, so a browse reads only the requested page instead of the composite walking and sorting the whole catalogue. `_summary` listings no longer read or JSON-parse the `compose` blob. ValueSet browse latency drops ~50× (`_count=10`: 34 ms → 0.7 ms in process; ~12× higher throughput at 50 VU).
* `Bundle.total` is now omitted from a paginated searchset (permitted by FHIR): across multiple providers an exact deduplicated count can't be had without a full key enumeration, and summing per-provider counts would double-count shared canonicals. It is still reported when the result fits in one page (and for `_summary=count` / `_count=0`).
* FTRM CodeSystem/ConceptMap point operations (`$lookup`, `$validate-code`, `$subsumes`, `$translate`) resolve an unversioned reference to its SemVer-latest version in O(1) via a precomputed `url → entry` index, instead of scanning the whole metadata cache on every version-less request.
