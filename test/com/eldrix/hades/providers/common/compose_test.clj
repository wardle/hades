...
(deftest expand-compose-unknown-system-error-test
  (testing "multi-system include with unknown CS emits error (expand purpose)"
    (let [compose {"include" [{"system" "http://example.com/cs"}
                              {"system" "http://unknown.example.com/loinc"}
                              {"system" "http://unknown.example.com/rxnorm"}]}
          {:keys [issues]} (compose/expand-compose *svc* compose {:purpose :expand})]
      (is (= 2 (count issues)))
      (is (every? #(= "error" (:severity %)) issues))
      (is (every? #(= "not-found" (:details-code %)) issues))
      (is (every? #(re-find #"value set cannot be expanded" (:text %)) issues))
      (is (some #(re-find #"loinc" (:text %)) issues))
      (is (some #(re-find #"rxnorm" (:text %)) issues))))
  (testing "validate purpose does not emit unknown-system issues"
    (let [compose {"include" [{"system" "http://unknown.example.com/loinc"}]}
          {:keys [issues]} (compose/expand-compose *svc* compose {})]
      (is (empty? (filter #(re-find #"loinc" (:text %)) issues))))))
