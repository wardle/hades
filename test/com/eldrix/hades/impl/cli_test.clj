...
(def cli-tests
  [{:s    "Legacy download command line arguments (pair form)"
   ...
   {:s    "install without --db no longer raises a parse-time --db error"
    :args ["install" "--dist" "uk.nhs/sct-clinical" "--dist" "uk.nhs/sct-drug-ext"]
    :test (fn [parsed]
            (is (= [] (get-in parsed [:options :db])))
            (is (not-any? #(re-find #"--db" %) (or (:errors parsed) []))
                "missing-db is enforced at command entry, not at parse time"))}
   ...
   {:s    "index"
    ...
    :test (fn [{:keys [cmds options]}]
            (is (= ["index"] cmds))
            (is (= ["snomed.db"] (:db options))))}
   {:s    "status without --db parses cleanly (mandatory check happens at command entry)"
    :args ["status"]
    :test (fn [{:keys [errors options]}]
            (is (nil? errors))
            (is (= [] (:db options))))}
   {:s    "status with --db"
   ...
   {:s    "serve with --port parsed as number; serve's --db is repeatable"
   ...
   {:s    "serve accepts repeated --db"
    :args ["serve" "--db" "snomed.db" "--db" "loinc.db" "--db" "fhir.db"]
    :test (fn [{:keys [options errors]}]
            (is (nil? errors))
            (is (= ["snomed.db" "loinc.db" "fhir.db"] (:db options))))}
   {:s    "serve accepts repeated --resources"
    :args ["serve" "--resources" "/a" "--resources" "/b"]
    :test (fn [{:keys [options errors]}]
            (is (nil? errors))
            (is (= ["/a" "/b"] (:resources options))))}
   {:s    "serve with positional source paths"
    :args ["serve" "snomed.db" "loinc.db"]
    :test (fn [{:keys [arguments errors]}]
            (is (nil? errors))
