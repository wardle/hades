...
(deftest preferred-terms-end-to-end
  (testing "preferred-terms returns a map keyed by concept-id"
    (let [db-path (try (snomed-db/assert-pinned-db!)
                       (catch Exception _ nil))]
      (if-not db-path
        (println "Skipping — pinned SNOMED DB not present. Run clj -X:build-db.")
        (with-open [svc (hermes/open db-path)]
