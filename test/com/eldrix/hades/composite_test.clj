(ns com.eldrix.hades.composite-test
...
(def ^:private cm-a-to-b-conflicting
  (assoc-in cm-a-to-b-alt ["group" 0 "element" 0 "target" 0 "code"] "Y"))

(def ^:private cm-b-to-b-alt
  (-> cm-a-to-b-alt
      (assoc-in ["group" 0 "element" 0 "code"] "B")
      (assoc-in ["group" 0 "element" 0 "target" 0 "code"] "Y")))

(def ^:private cm-vs-declared-a-to-b
  {"resourceType" "ConceptMap"
   "url" "http://example.com/r/cm-vs-declared" "version" "1.0" "status" "active"
   "sourceCanonical" "http://example.com/r/vs-source"
   "targetCanonical" "http://example.com/r/vs-target"
   "group" [{"source" "http://example.com/r/cs"
             "target" "http://example.com/r/cs-other"
             "element" [{"code" "A"
                         "target" [{"code" "X" "equivalence" "equivalent"}]}]}]})
...
(deftest cm-translate-uses-group-systems-when-top-level-declares-valuesets
  (let [svc (svc-of [cs-v1 cs-other cm-vs-declared-a-to-b])]
    (testing "URL-driven translate works when ConceptMap.source/target are ValueSets"
      (let [result (hades/translate svc {:url "http://example.com/r/cm-vs-declared"
                                         :code "A"})]
        (is (true? (:result result)))
        (is (= "X" (-> result :matches first :code)))))
    (testing "system+target selection matches group CodeSystem pairs"
      (let [result (hades/translate svc {:system "http://example.com/r/cs"
                                         :target "http://example.com/r/cs-other"
                                         :code "A"})]
        (is (true? (:result result)))
        (is (= "X" (-> result :matches first :code)))))
    (testing "system+target selection accepts the ConceptMap target ValueSet scope"
      (let [result (hades/translate svc {:system "http://example.com/r/cs"
                                         :target "http://example.com/r/vs-target"
                                         :code "A"})]
        (is (true? (:result result)))
        (is (= "X" (-> result :matches first :code)))))))

(deftest cm-translate-ambiguous-system-target-test
  (let [svc (svc-of [cs-v1 cs-other cm-a-to-b cm-a-to-b-conflicting])
        result (hades/translate svc {:system "http://example.com/r/cs"
                                     :target "http://example.com/r/cs-other"
                                     :code "A"})]
    (testing "conflicting ConceptMaps for the same code force :url"
      (is (false? (:result result)))
      (is (some #(= "ambiguous-target" (:details-code %)) (:issues result))))))

(deftest cm-translate-disambiguates-multiple-candidates-by-code-test
  (let [svc (svc-of [cs-v1 cs-other cm-a-to-b cm-b-to-b-alt])
        result (hades/translate svc {:system "http://example.com/r/cs"
                                     :target "http://example.com/r/cs-other"
                                     :code "A"})]
    (testing "when only one matching ConceptMap contains the requested code, use it"
      (is (true? (:result result)))
      (is (= "X" (-> result :matches first :code))))))

(deftest cm-translate-no-match-test
  (let [svc (svc-of [cs-v1 cs-other cm-a-to-b])]
    (testing "no matching ConceptMap returns a structured failure"
      (let [result (hades/translate svc {:url "http://example.com/r/cm-missing"
                                         :code "A"})]
        (is (false? (:result result)))
        (is (:not-found result))
        (is (some #(= "not-found" (:details-code %)) (:issues result)))))))
