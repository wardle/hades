(ns com.eldrix.hades.ftrm-vs-inmemory-bench
  "One-shot comparison of the FTRM container vs the in-memory provider
  ...
  Run:
    ...
    clj -X:bench :nses '[com.eldrix.hades.ftrm-vs-inmemory-bench]'
    ;; or in REPL:
    (require 'com.eldrix.hades.ftrm-vs-inmemory-bench :reload)
    (com.eldrix.hades.ftrm-vs-inmemory-bench/compare-providers!)"
...
(defn- build-sqlite! []
  ...
  (let [load (time-ms (vec (fhir-loader/load-paths terminology-dir)))
        data (:result load)
        build (time-ms (ftrm-index/build! out-db data {:loader-type "fhir-r4-defs"}))
        index (time-ms (ftrm-index/index! out-db))
...
(defn- bench-sqlite []
  (let [build (build-sqlite!)
        open (time-ms (ftrm-provider/open-providers out-db))
