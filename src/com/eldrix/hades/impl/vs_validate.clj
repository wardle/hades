(ns com.eldrix.hades.impl.vs-validate
  ...
  Provider impls (`MemoryValueSet`, `SqliteValueSetCatalogue`, …) call
...
(defn- find-best-match
  ...
  on the returned match when the only match is case-insensitive."
  [expanded code system display caller-version multi-version-systems]
...
(defn validate-code
  ...
  (let [expanding (conj (or (:expanding params) #{}) url)
        ...
        compose-result (compose/expand-compose svc compose compose-params)
    ...
    (cond
      ...
      :else
      (let [match (-> (find-best-match expanded code system display caller-version multi-version-systems)
