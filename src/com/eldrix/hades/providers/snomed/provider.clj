(ns com.eldrix.hades.providers.snomed.provider
...
(defn- compose-filter->query
  ...
  (cond
    ...
    (if-let [cid (parse-long (str value))]
      (case op
        "=" (hermes.search/q-concept-id cid)
        (not-supported-issue (str "filter property=code op=" op " not supported")))
      (value-issue property op value))
    ...
    (when-not (and (= op "=") (contains? #{"true" "false"} (str value)))
...
(deftype HermesService
  ...
  (cs-subsumes
    ...
    (let [codeA' (try (Long/parseLong codeA) (catch NumberFormatException _ nil))
          codeB' (try (Long/parseLong codeB) (catch NumberFormatException _ nil))]
      (cond
        (or (nil? codeA') (nil? (hermes/concept svc codeA')))
        (issues/unknown-code-subsumes systemA codeA "codeA")
        (or (nil? codeB') (nil? (hermes/concept svc codeB')))
        (issues/unknown-code-subsumes systemB codeB "codeB")
        :else
        {:outcome
         (cond
